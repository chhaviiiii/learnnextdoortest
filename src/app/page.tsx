      ...
      <section className="relative overflow-hidden">
        ...
        <div className="mx-auto max-w-[1240px] px-6 pb-16 pt-14 md:pt-20">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <form action="/browse" method="get" className="relative mx-auto mt-8 max-w-xl">
              <Search className="absolute left-5 top-1/2 h-5 w-5 -translate-y-1/2 text-ink-500" />
              <input
                name="q"
                placeholder="Try 'guitar', 'pottery', 'chess for kids'..."
                className="w-full rounded-2xl border-0 bg-white py-4 pl-14 pr-36 text-sm text-ink-900 shadow-float ring-1 ring-ink-800/5 focus:outline-none focus:ring-2 focus:ring-brand-400"
              />
              <button
                type="submit"
          ...
          <div className="mx-auto mt-14 grid max-w-3xl grid-cols-2 gap-4 md:grid-cols-4">
            {[
              { icon: Users, label: "5,000+ learners" },
              ...
              <div
                key={i}
                className="flex items-center gap-2 rounded-2xl bg-white/80 px-4 py-3 ring-1 ring-ink-800/5"
              >
                <s.icon className="h-4 w-4 text-brand-600" />
                <span className="text-xs font-semibold text-ink-800">{s.label}</span>
              </div>
            ))}
          </div>
        </div>
